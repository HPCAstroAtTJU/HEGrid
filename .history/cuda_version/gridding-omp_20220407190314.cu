...
void init_input_with_cpu(const int &sort_param) {
    ...
    if (sort_param == BLOCK_INDIRECT_SORT) {
        ...
        boost::sort::block_indirect_sort(V.begin(), V.end(),2);
    ...
    h_hpx_idx = RALLOC(uint64_t, data_shape+1);  // 这里修改了
...
void data_alloc(){
    uint32_t data_shape = h_GMaps.data_shape;
    uint32_t channels = h_GMaps.spec_dim;
    printf("data_shape=%d, channels=%d\n", data_shape, channels);
    uint32_t num = h_zyx[0] * h_zyx[1] * h_zyx[2];
...
/*********************************************/ 
    ...
    double iTimeK1 = cpuSecond();
    dim3 block(bDim);
    dim3 grid((h_GMaps.block_warp_num * h_zyx[2] - 1)/ (block.x / 32) + 1);
    // Get start time.
    ...
    float kernel_time;
/***************************kernel_time*********************************/
    HANDLE_ERROR(cudaEventElapsedTime(&kernel_time, start, stop));
/**********************************************************************/

    printf("%f, ", kernel_time);
    double iTimeK2 = cpuSecond();
